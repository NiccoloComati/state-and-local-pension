...
p <- ggplot() + 
  geom_line(data = actual_data, aes(x = fy, y = fr), colour = "black")

# Add the forecast line
p <- p + geom_line(data = forecast_data, aes(x = fy, y = Forecast), colour = "blue")

# Add the confidence intervals with `geom_ribbon`
p <- p + geom_ribbon(data = forecast_data, aes(x = fy, ymin = Lo95, ymax = Hi95), alpha = 0.1) # 95% CI
p <- p + geom_ribbon(data = forecast_data, aes(x = fy, ymin = Lo80, ymax = Hi80), alpha = 0.2) # 80% CI

# Labels and theme
p <- p + labs(title = paste0("Forecasted Funding Ratio For ",plan),
              x = "Fiscal Year",
              y = "Funding Ratio") +
  theme_minimal()

# Print the plot
ggsave(paste0("Results/Output/Forecast_Plots/",plan,"_forecast_plot.pdf"), plot = p, width = 10, height = 6, dpi = 300)
