# ============================================================================
# bucketfill_cf_model_072026.R
#
# Copy of Common_Code/bucketfill_cf_model.R carrying ONE correction, to
# LinearFill's within-band weight (see the block inside that function).
#
# Why a separate file: Common_Code/bucketfill_cf_model.R is shared by both
# cluster lineages. cluster_code_2022 must keep producing exactly what it
# always produced, so it is left untouched and keeps sourcing the original.
# Only cluster_code_2022_072026 sources this file.
#
# Created 2026-07-30. Mirrors the same correction made in
# Code/python/bucketfill_cf_model.py.
# ============================================================================

# Strength of the within-band tilt used by LinearFill (see the correction block
# inside that function). Weights run from (1 + s) to (1 - s) across a band, so
# s = 0 is an exact even split and s shifts a band's mean age by exactly s years.
# Must stay below 1 or weights could turn negative. Matches LINEARFILL_TILT in
# Code/python/bucketfill_cf_model.py - keep the two in step.
      ...
      for (k in c(1:N)){
        ...
        for(L in c(1:M)){
          ...
          if((columnmin + L - 1) > svcmax){
            ...
            # ----------------------------------------------------------------
            # ORIGINAL WEIGHT (unchanged in Common_Code/bucketfill_cf_model.R).
            # CORRECTED 2026-07-30. Left commented out because it produced every
            # result up to and including the 072026 run and must stay legible.
            #
            #     Share[k,L] <- GroupCount/(N*M) + Slope*(rowmin + k-1 )
            #
            # Three defects:
            #  1. It subtracts an AGE IN YEARS from a HEADCOUNT PER CELL. Two
            #     different quantities; the difference has no meaning.
            #  2. GroupCount sits inside the weight, so the size of the tilt
            #     depends on how big the plan is. A band of 20,000 came out
            #     almost flat (0.1%), a band of 500 came out at 17%.
            #  3. The weights are centred on (GroupCount/N - mean age) rather
            #     than on 1, so they straddle zero. Their sum, used as the
            #     normaliser below, equals GroupCount minus the sum of the ages
            #     in the band. When a band holds about that many people the
            #     normaliser approaches zero and the division explodes; either
            #     side of that point the tilt silently REVERSES direction.
            #
            # Measured: 3 of 40 plans produced NEGATIVE retiree headcounts.
            # OK134's 75-79 band held 385.004 people against an age-sum of 385,
            # so the normaliser was 0.004 and the band filled with +/-200,000
            # people in a plan with 4,242 retirees. Band totals still reconciled
            # because the positives and negatives cancel, which is why it went
            # unnoticed. A one-person data revision moved that plan's liability
            # by 56%.
            #
            # CORRECTED WEIGHT: a perturbation around 1, driven only by POSITION
            # in the band, never by the headcount and never by the absolute age.
            #   u runs 0 -> 1 across the band
            #   w runs (1+s) -> (1-s) for Slope = -1 (retirees, declining)
            #     and (1-s) -> (1+s) for Slope = +1 (actives, rising)
            # All weights strictly positive for s < 1, so no cell can go
            # negative, and they sum to exactly N, so the normaliser is constant
            # and can never approach zero. s = 0 gives an even split.
            # ----------------------------------------------------------------
            u <- if (N == 1) 0 else (k - 1)/(N - 1)
            Share[k,L] <- 1 + Slope * LINEARFILL_TILT * (2*u - 1)
