...
#If contributrions are NA then fix them
if(is.na(EmployeeContributionRate)){
  EmployeeContributionRate <- (planinfo$contrib_EE_regular*1000)/sum(active*BaseWage[,,1])
  
}

if(is.na(EmployerContributionRate)){
  
  EmployerContributionRate <- (planinfo$contrib_ER_regular*1000)/sum(active*BaseWage[,,1])
...
# in 2017, then take the data from them. If it is inacurate then the number of inatives
# is a fraction of the total of inactives based on Brookings reporting
