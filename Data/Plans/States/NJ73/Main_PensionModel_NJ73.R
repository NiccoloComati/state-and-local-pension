...
availableData <- c(T,T,T,F,T,T,F,F,F)
...
#library("readxl")
#library("readxl", lib.loc=.libPaths("/mmfs1/data/smithafe/R/x86_64-pc-linux-gnu-library/4.2"))
#library("vctrs", lib.loc=.libPaths("/mmfs1/data/smithafe/R/x86_64-pc-linux-gnu-library/4.2"))
#get other code 
...
# The return that employees who leave with their money get
refundReturn <-  PPD$inflation

## (DYNAMIC) Inflation Rate
Inflation <-  PPD$inflation


WageGrowth <-  PPD$wage_inf
...
EmployeeContributionRate <- PPD$employeecont
...
setCurrentTier <- F
if(num_tiers==1){setCurrentTier <- T}
MainRes_Tier1 <- Main_Current(active_t1,inactive_t1,COLA_t1,WageYears_t1,BenefitCap_t1,
                              BenefitFactor_t1,RetirementStart_t1,NyearFullBenefit_t1,CurrentTier=setCurrentTier)

#Check if there are more tiers. If there are run the code. If not, create data with all zeros
if(num_tiers >= 2){
  if(num_tiers==2){setCurrentTier <- T}
MainRes_Tier2 <- Main_Current(active_t2,inactive_t2,COLA_t2,WageYears_t2,BenefitCap_t2,
                              BenefitFactor_t2,RetirementStart_t2,NyearFullBenefit_t2,CurrentTier=setCurrentTier)
}else{
  
  MainRes_Tier2 <-list(array(0,dim(MainRes_Tier1[[1]])),array(0,dim(MainRes_Tier1[[2]])),
...
#Check if there are more tiers. If there are run the code. If not, create data with all zeros
if(num_tiers >= 3){
  if(num_tiers==3){setCurrentTier <- T}
  MainRes_Tier3 <- Main_Current(active_t3,inactive_t3,COLA_t3,WageYears_t3,BenefitCap_t3,
                                BenefitFactor_t3,RetirementStart_t3,NyearFullBenefit_t3,CurrentTier=setCurrentTier)
}else{
  
  MainRes_Tier3 <-list(array(0,dim(MainRes_Tier1[[1]])),array(0,dim(MainRes_Tier1[[2]])),
                       array(0,dim(MainRes_Tier1[[3]])),array(0,dim(MainRes_Tier1[[4]])))
...
#Check if there are more tiers. If there are run the code. If not, create data with all zeros
if(num_tiers >= 4){
  if(num_tiers==4){setCurrentTier <- T}
  MainRes_Tier4 <- Main_Current(active_t4,inactive_t4,COLA_t4,WageYears_t4,BenefitCap_t4,
                                BenefitFactor_t4,RetirementStart_t4,NyearFullBenefit_t4,CurrentTier=setCurrentTier)
}else{
  
  MainRes_Tier4 <-list(array(0,dim(MainRes_Tier1[[1]])),array(0,dim(MainRes_Tier1[[2]])),
                       array(0,dim(MainRes_Tier1[[3]])),array(0,dim(MainRes_Tier1[[4]])))
...
if(num_tiers >= 5){
  ...
  if(num_tiers==5){setCurrentTier <- T}
  MainRes_Tier5 <- Main_Current(active_t5,inactive_t5,COLA_t5,WageYears_t5,BenefitCap_t5,
                                BenefitFactor_t5,RetirementStart_t5,NyearFullBenefit_t5,CurrentTier=setCurrentTier)
}else{
  
  MainRes_Tier5 <-list(array(0,dim(MainRes_Tier1[[1]])),array(0,dim(MainRes_Tier1[[2]])),
                       array(0,dim(MainRes_Tier1[[3]])),array(0,dim(MainRes_Tier1[[4]])))
...
if(num_tiers >= 6){
  ...
  if(num_tiers==6){setCurrentTier <- T}
  
  MainRes_Tier6 <- Main_Current(active_t6,inactive_t6,COLA_t6,WageYears_t6,BenefitCap_t6,
                                BenefitFactor_t6,RetirementStart_t6,NyearFullBenefit_t6,CurrentTier=setCurrentTier)
}else{
  
  MainRes_Tier6 <-list(array(0,dim(MainRes_Tier1[[1]])),array(0,dim(MainRes_Tier1[[2]])),
                       array(0,dim(MainRes_Tier1[[3]])),array(0,dim(MainRes_Tier1[[4]])))
...
#save.image(file = paste0(plan,"/",plan,"_Compare_",date_run,".RData"))
