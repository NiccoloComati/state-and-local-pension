...
#DisabilityPayoutRate <- 0.00463548527731356
...
discountrate <- PPD$discount #blended
...
availableData <- c(T,T,T,F,T,T,T,F,F)
...
t1_pvfb_model <- sum(MainRes_Tier1[[4]][1,])


t2_pvfb_model <- sum(MainRes_Tier2[[4]][1,])



model_PVFB <- t1_pvfb_model+t2_pvfb_model+RetRes[[1]][1,1]
...
t1_inactive_model <- sum(MainRes_Tier1[[4]][1,2])


t2_inactive_model <- sum(MainRes_Tier2[[4]][1,2])

inactive_model <- t1_inactive_model+t2_inactive_model
...
save.image(file = "AZ127/AZ127_Compare_12062023.RData")
