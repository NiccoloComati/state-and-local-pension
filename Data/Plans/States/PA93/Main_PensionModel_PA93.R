...
DSdir <- "/home/dan/golub/Pension/Pension_CF_Model/"

planFolder <- "PA93/"

fileName <- "PA93_2017.xlsx"
...
DisabilityPayoutRate <-  133374698/5984083555
...
if(availableData[[7]]){
  ...
  asy_retrate <- as.matrix(read_excel("Common_Data/default_assumptions.xlsx",
                                 sheet = "retirement", range = "Q2:AA12",col_names = F))
}
...
if(availableData[[6]]){
  ...
  asy_seprate <- as.matrix(read_excel("Common_Data/default_assumptions.xlsx",
                                         sheet = "withdrawal", range = "B2:L12",col_names = F))
...
AnnuityVector <- ComputeAnnuity(COLA_annuity)
...
if(availableData[[2]]){
  ...
  ret_num <- matrix(read_excel("Common_Data/default_assumptions.xlsx",
                               sheet = "retdist", range = "B1:B17",col_names = T))*planinfo$sys_beneficiaries_tot
  
  ret_ben <- matrix(read_excel("Common_Data/default_assumptions.xlsx",
                               sheet = "retdist", range = "F1:F17",col_names = T))*planinfo$BeneficiaryBenefit_avg*1000
...
totalwage <- sum(active*BaseWage[,,1])
EmployeeContributionRate <- planinfo$contrib_EE_regular*1000 / totalwage
EmployerContributionRate <- planinfo$contrib_ER_regular*1000 / totalwage
...
#Calculate the inactive matrix
#assume inactive receive the most recent tier benefits
inactive <- Calc_Inactive(active, SeparationRate, RefundRate,
                         MortalityTable,RetirementStart_t2,NyearFullBenefit_t2)


inactive <- inactive * planinfo$InactiveVestedMembers
...
#Main_Current takes:
# 1. active  age/serv matrix
# 2. inactive age/serv matrix
...
MainRes_Tier3 <- Main_Current(active_t3,inactive_t3,COLA_t3,WageYears_t3,BenefitCap_t3,
                              BenefitFactor_t3,RetirementStart_t3,NyearFullBenefit_t3,CurrentTier=T)
...
#Sum Cash Flows
main_cf <- MainRes_Tier1[[2]]  + MainRes_Tier2[[2]] +MainRes_Tier3[[2]]

ret_cf <- RetRes[[2]]

cash_outflows <- main_cf+ret_cf

cash_inflows <- MainRes_Tier1[[3]]  + MainRes_Tier2[[3]]+ MainRes_Tier3[[3]]


#Liabilities
# This is the acturial accrued liabilities
# The first row in this data frame should match what is reported in the CAFR
AAL <- MainRes_Tier1[[1]]+MainRes_Tier2[[1]]+RetRes[[1]]+ MainRes_Tier3[[1]]
...
save.image(file = "PA93/Data/PA93_101723.RData")
