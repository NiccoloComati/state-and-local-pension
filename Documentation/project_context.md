...
3. **Load** master PPD database and extract plan row (`ppd-data-latest_072026.xlsx`)
...
**The three plans of the 40 that the canonical run does not cover** (37 modeled). Investigated 2026-07-29 against the **Python** engine, which is a single universal runner: a plan is admitted by a 9-boolean row in the `AVAILABLE_DATA` dict of `Code/python/fast/Main_PensionModel.py` (plus optional membership in `CONTRIB_RATE_NA_CHECK` / `RETDIST_SKIPROWS`). There are no per-plan Python scripts, so "no script exists" is not a blocker for anything. The dict was ported from the 38 R **2022 cluster** scripts, which are the set that omits MA51 and MO64.

All three have a `[PLAN]_2017.xlsx` workbook, a tier row, a `PPD_planlevel_main_updated.csv` row, and an `availableData` vector declared in their own 2017 R script.

- `MO64` — declared `availableData = c(T,T,T,T,T,T,T,F,F)`, identical to OK134's. Its workbook is equivalent to OK134's on **every range the engine reads**: same sheet list, same shapes, same NaN pattern, `ageservice` sums to 1.000000, `retdist` to 1.000000. PPD complete on every engine-read field at FY2017, FY2022 **and** FY2023. `inactive_adj = 1.0` with legacy `inactive = 8500` available. **No data or layout obstacle found.**
- `MA51` — declared `availableData = c(T,T,T,T,T,T,F,F,F)`; the `refund` sheet is absent and `retirement` sits in a non-standard position, both already flagged `F`. `inactive_adj = 0.0`, so `scale_inactive_members` returns a zero inactive population by construction and the missing `InactiveVestedMembers` never fires (legacy `inactive` is also 0). **Open blocker:** `contrib_ER_regular` is missing in PPD at FY2017, FY2022 and FY2023, so `EmployerContributionRate` is NaN; the `CONTRIB_RATE_NA_CHECK` recomputation reads the same missing field and would not help. An employer contribution rate has to come from somewhere else.
- `MA50` — PPD complete at FY2017 and FY2022. Its two scripts **disagree on data availability**: the 2017 top-level script declares `c(T,T,T,T,T,T,T,F,F)`, the 2022 cluster script `c(T,T,T,T,T,F,F,F,F)`, dropping `withdrawal` and `retirement` in favour of defaults. The 2022 choice is consistent with the workbook: MA50's `withdrawal` block uses non-standard age bins (20, 30, 40, 45, 50, …) and has 10 age rows where the generic read expects 11.

**Layout quirk shared by MA50 and MA51:** the `wagerel` block has only 10 age rows (25–70); OK134/MO64 have 11 (25–75). The generic read (`B2:L12`, identical in all four scripts) therefore over-runs the data. R pads the missing row with `NA`; pandas returns a (10, 11) array and `ConstantFill` silently leaves the top age bin at zero. Neither behaviour is obviously correct — this needs a decision before either plan is admitted. MA50's `wagerel` is additionally degenerate: every service column within an age row carries the same value, i.e. its wage relativities vary by age only.

**Corrections to earlier claims about MA50** (each re-checked against the scripts on 2026-07-29):
- The `++` in `MainRes_Tier5[[2]]++ MainRes_Tier6[[2]]` (2022 script, line 714) is **not a syntax error**. In R, `a ++ b` parses as `a + (+b)`; verified under R 4.4.1 that the result is `identical()` to `a + b`. Cosmetic only.
- "Backward tier logic" applies to the 2017 script only; the 2022 script calls `CreateTiers(active, inactive, num_tiers)` exactly as OK134 does.
- "Missing the `*1000` asset multiplier" is not MA50-specific: `Assets[1,] <- planinfo$ActAssets_GASB` is identical in OK134 and MA50 in both lineages, and the Python engine applies `*1000` uniformly.
- "Different risk-free-rate formula" is not supported: `rf <- .01` is identical in both scripts, and Python uses `0.01 + Inflation` for every plan.
- "Produces no `NormalCost`" is a property of the 2017 script generation — OK134's 2017 script computes none either.
