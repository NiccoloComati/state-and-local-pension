...
**Created:** 2026-06-11
**Purpose:** Every piece of data the production engine consumes — what it is, where the code reads it from, which source channel it ultimately comes from, the fallback chain, and the hard-coded constants. This is the "schema" side of the provenance catalogue; the per-plan "instance" side is `Documentation/provenance/provenance_register.csv` (generated by `Documentation/provenance/provenance_scan.py`).

**Authority:** derived by reading the production code (`Code/python/engine/run_plan.py`, `Code/python/engine/functions.py` helpers, `Code/python/asset_simulation.py`), not from prior documentation. Code citations are to the fast runner unless noted.
...
`Code/python/engine/run_plan.py` runs for **every plan, every run**:
...
| Model variable | Source expression | Fallback chain (from `engine/functions.py` helpers) |
|---|---|---|
