...
- **sd (run 2026-07-13, raw 0.25 as expected):** publishes separation per GROUP (General/Safety); the workbook blends them with AGE-VARYING headcount weights (blend differs per age row). Blending needs the cross-table weighted op (rung 3) with an age x group headcount weights table — the same op shape Avg_Mort needs. **Update 2026-07-14: op built (`group_weighted`, v0.6) and the collector's exact recipe reverse-engineered:** she used JOINT (age-bin x service-bin) headcounts from Tables A-9/A-11 (e.g. a(25, svc 1-4) = 27/(27+194) = 0.1222 — confirmed exactly), with each template service column taken from a SINGLE source year (col '6' <- year 6, NOT the template's [5,6] average — more single-year evidence for the col-semantics question above). The executor reproduces her blend on 98/110 cells; ALL residuals are cols '30'/'40', where she weighted by the single count bucket containing year 30/40 while the op weights by the value's source-bin span ('20+'). Which weight-bucket convention is canon for aggregate service columns = part of this entry's ruling. Both variants re-derivable from archived tables.
...
### 6c. Retirement (retdist) sheet conventions — adopted from the collectors (pending confirmation)
- **Which payees count as "retirees":** phx F.6 splits pay statuses, and the collector used SERVICE RETIREES only (excluding disabled and beneficiaries/QDROs — the same population choice as the Avg_Mort weights, entry 6b). But mil's "Summary of Retired Members" tables have no such split, and the collector took them wholesale — documents define "retired members" differently, so the extracted population is only as consistent as the sources. Rule adopted: use service retirees where split, the document's retiree exhibit as-is where not, and say which in notes.
- **Even bucket splits:** a printed age bucket covering several template rows is split EVENLY — counts and dollars each divided by the number of covered rows, so each row carries the bucket's own average (phx '90 & Up' /3; mil '59 & Under' /2 AND '90 & Over' /3; sd 90+ /3; the mil collector's inline note says "split the age buckets evenly when needed"). Alternatives (all in 90-94; mortality-informed splits) were never used by any collector.
- **Monthly → annual = ×12:** mil prints MONTHLY benefits (the December payment); the workbook embeds ×12. Adopted as the declared `annualize_monthly` op. **Caveat:** mil pays pension escalators/13th-check-style increases (AV Appendix B), so December-payment × 12 is the collector's approximation, not an identity — worth a ruling.
- **Bottom bucket clip:** '<55' → template '50-54' (same clip-to-plausible-ages judgment as the Avg_Mort weight bins).
- All conventions are declared per-run (share_even / ratio / annualize_monthly visible in the ops summary) and re-derivable from archived transcriptions if ruled otherwise.
...
- **Pipeline gap (closed 2026-07-14, v0.5):** the contract now has an explicit `unavailable` declaration - empty maps, all-null derived grid, notes stating what the document publishes instead, the nearest related tables archived as evidence, and a prompt rule forbidding approximation of a missing dimension. This implements the flag-don't-accept ruling; it decides nothing about the data itself.
- **Options (the modeling decision, still open for the coauthor session):** (A) leave these grids empty (current pipeline behavior); (B) explicitly adopt age-only-within-age copying as a modeling assumption; (C) add another data source or model to estimate service variation; (D) add a separate age-only wage target/input so the evidence is preserved without pretending it is joint. Whatever is chosen can be applied from the archived evidence tables at zero API cost.
...
- **Avg_Mort construction (adopted from the verified phx recipe, 2026-07-14):** unisex column = population blend of the pre- and post-retirement sample rates, weights = actives/retirees counts-by-age; M/F simple average via a shared weight table per population; the post-disability panel is transcribed but EXCLUDED from the blend (disability is a separate model channel); each 5-year sample age stands for its band ([50,54] etc.); open-ended count bins used as weights are clipped to plausible ages and the clip declared in notes (actives '65 and over' → [65,69] — this is what makes 20-49 pre-only / 50-69 blend / 70+ post-only emerge from the data rather than a hard-coded segmentation). Ages beyond the printed sample table are NOT extrapolated (the phx workbook carries age-90 to 119 — entry 2's question again; 25 known residual cells).
